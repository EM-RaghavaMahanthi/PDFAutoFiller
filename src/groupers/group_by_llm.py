...
class GroupByLLM(BaseGrouper):
    ...
    {text}
    """
        logger.info("Started grouping radio buttons and handling them")
        raw_response = self.llm.complete(prompt)
        cleaned_json = re.sub(r"^```json\n?|```$", "", raw_response.text.strip(), flags=re.MULTILINE)
        try:
            parsed = json.loads(cleaned_json)
        except json.JSONDecodeError as e:
            print("Failed to parse LLM output as JSON. Output was:", repr(cleaned_json))
            raise
        return parsed
    ...
    def group(self):
        # Step 1: Extract relevant text lines around radio buttons
        lines = self.get_context_lines()
        # Step 2: Use LLM to group radio buttons from text
